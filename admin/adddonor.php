...
if (isset($_POST["submit"])) {
    ...
    $errors = array();
           ...
           if (empty($name) OR empty($email) OR empty($phoneno) OR empty($username) OR empty($password)) {
           ...
           if (!preg_match('/[!@#$%^&*()_+]/', $password)) {
            array_push($errors,"Password must have at least one symbol");
           }
           if (!preg_match('/[0-9]/', $password)) {
            array_push($errors,"Password must have at least one number");
           }
           if (!preg_match('/[a-zA-Z]/', $password)) {
            array_push($errors,"Password must have at least one alphabet");
           }
           ...
           if (count($errors)>0) {
            ...
            $sql3 = "INSERT INTO donor (name,email,phoneno,username,password) VALUES ( ?, ?, ?, ?, ?)";
            $stmt = mysqli_stmt_init($conn);
            $prepareStmt = mysqli_stmt_prepare($stmt,$sql3);
            if ($prepareStmt) {
                mysqli_stmt_bind_param($stmt,"sssss",$name, $email, $phoneno,$username,$password);
                mysqli_stmt_execute($stmt);
                echo '<script>alert("BERJAYA DITAMBAH")</script>';
                echo "<script type='text/javascript'> document.location ='adddonor.php'; </script>";
            }else{
...
<div class="container form-container">
    ...
    <form action="adddonor.php" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
       <div class="form-group">
            <label for="name"><i class="fas fa-users"></i> Nama:</label>
            <input type="text" class="form-control" name="name" placeholder="Nama Penderma" required="" title="Sila isi nama penuh seperti di dalam kad pengenalan" pattern="[a-zA-Z][a-zA-Z0-9\s]*">
        </div>
        <div class="form-group">
            <label for="email"><i class="fas fa-handshake"></i> Emel:</label>
            <input type="email" class="form-control" name="email" placeholder="Emel" required="" title="Sila masukkan emel yang sah" pattern="/^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <label for="username"><i class="fas fa-child"></i> Username:</label>
            <input type="text" class="form-control" name="username" placeholder="Username" required="required" pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*_=+-]).{8,20}" title="Sila isi username">
        </div>
        <div class="form-group">
            <label for="password"><i class="fas fa-user-check"></i> Password:</label>
            <input type="password" class="form-control" name="password" placeholder="Password" required="required" pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*_=+-]).{10}" title="Kata laluan tidak boleh melebihi 10 huruf dan termasuk sekurang-kurangnya 1 nombor dan 1 simbol ">
        </div>
